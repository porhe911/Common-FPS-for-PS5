/*
 * Common FPS v0.28b SR9B - static PUI visual bootstrap controller
...
extern "C" {
extern const unsigned char commonfps_sr9b_receiver_elf[];
extern const std::size_t commonfps_sr9b_receiver_elf_size;
...
constexpr const char* kLogPath = "/data/CommonFPS_SR9B_static_visual.log";
...
constexpr std::uint16_t kHealthPortNetwork = 0xF6D8U;
constexpr std::uint32_t kLoopback = 0x0100007FU;
constexpr std::uint32_t kHealthMagic = 0x42394852U;
constexpr std::uint32_t kHealthReady = 1U;
constexpr std::uint32_t kHealthPacket = 2U;
constexpr std::uint32_t kHealthContext = 3U;
constexpr std::uint32_t kHealthVisual = 4U;
constexpr std::uint64_t kContextFullMask = 0x7FFFULL;
constexpr std::uint32_t kVisualReadyStage = 5U;
...
void log_injection(const common_fps::legacy_v028b::InjectionResult& r) noexcept {
    ...
    std::fprintf(f,
        "SR9B INJECT attached=%d elf_loaded=%d payload_args=%d bootstrap=%d pthread_ok=%d pthread_rc=%d trace_continue=%d trace_stop=%d imports_resolved=%u imports_unresolved=%u first_unresolved=%s\n",
...
void log_health(const HealthPacket& h) noexcept {
    FILE* f = std::fopen(kLogPath, "a");
    if (!f) return;
    if (h.kind == kHealthContext) {
        std::fprintf(f, "SR9B CONTEXT stage=%u mask=0x%04llx\n",
            h.loading, static_cast<unsigned long long>(h.sequence));
    } else if (h.kind == kHealthVisual) {
        std::fprintf(f, "SR9B VISUAL stage=%u mask=0x%04llx\n",
            h.loading, static_cast<unsigned long long>(h.sequence));
    } else if (h.kind == kHealthReady) {
        std::fprintf(f, "SR9B VISUAL READY stage=%u mask=0x%04llx\n",
            h.loading, static_cast<unsigned long long>(h.sequence));
    } else if (h.kind == kHealthPacket) {
        std::fprintf(f, "SR9B HEALTH seq=%llu state=%s fps=%.1f\n",
...
public:
    ...
    bool wait_visual_ready(unsigned milliseconds) noexcept {
        for (unsigned elapsed = 0; elapsed < milliseconds; elapsed += 20) {
            HealthPacket h{};
            const ssize_t got = recvfrom(health_, &h, sizeof(h), MSG_DONTWAIT,
                                         nullptr, nullptr);
            if (got == static_cast<ssize_t>(sizeof(h)) && h.magic == kHealthMagic) {
                log_health(h);
                if (h.kind == kHealthReady &&
                    h.sequence == kContextFullMask &&
                    h.loading == kVisualReadyStage) {
                    return true;
                }
            }
            usleep(20000);
...
private:
    void send(const PhufStatePacket& p) noexcept {
        ...
        if (n != static_cast<ssize_t>(sizeof(p))) {
            log_line("SR9B PHUF send failed");
            return;
        }
        ...
        log_line("SR9B HEALTH packet timeout");
    }
...
int run_probe() noexcept {
    ...
    log_pid("SR9B CHILD pid=", getpid());
    log_line("SR9B START static PUI visual probe");
    log_line("SR9B VISUAL static only: FPS: + loading; NO dynamic text updates / NO Detour / NO UI hook");
    log_line("SR9B backend+PHUF+bootstrap identical to proven SR9A");

    Wire wire;
    if (!wire.initialize()) {
        log_line("SR9B ABORT controller UDP init failed");
        return 1;
    }

    const pid_t shellui = wait_stable_shellui();
    log_pid("SR9B ShellUI stable pid=", shellui);
    if (shellui <= 0) return 2;

    const auto injection = inject_renderer_once(
        shellui, commonfps_sr9b_receiver_elf, commonfps_sr9b_receiver_elf_size);
    log_injection(injection);
    if (commonfps_v028b_import_unresolved_count() != 0) {
        log_line("SR9B ABORT fail-closed import resolver rejected receiver");
        return 3;
    }
    if (!injection.pthread_create_ok) {
        log_line("SR9B ABORT injection did not reach successful pthread_create");
        return 4;
    }
    if (!wire.wait_visual_ready(12000)) {
        log_line("SR9B ABORT static PUI visual did not reach READY");
        return 5;
    }

    log_line("SR9B STATIC VISUAL PASS; begin unchanged production PHUF lifecycle");
    ...
    for (int second = 0; second < kRuntimeSeconds; ++second) {
        ...
        if (pid != active_pid) {
            ...
            log_pid("SR9B CHANGE eboot.bin pid=", active_pid);
        }
        if (active_pid <= 0) { wire.loading(); sleep(1); continue; }
        ++seconds_on_pid;
        ...
        if (elapsed > 0) {
            ...
            if (warmup) { --warmup; wire.loading(); }
            else if (tenths > kMaxTenthsFps) wire.loading();
            else wire.fps(static_cast<double>(tenths) / 10.0);
        } else {
            wire.loading();
        }
    ...
    log_line("SR9B DONE controller clean return after 240s; static receiver remains until reboot");
    return 0;
}
}

extern "C" int main() {
    log_pid("SR9B PARENT start pid=", getpid());
    const pid_t child = fork();
    if (child > 0) {
        log_pid("SR9B PARENT forked child=", child);
        log_line("SR9B PARENT RETURN 0");
        return 0;
    }
    if (child < 0) log_line("SR9B fork failed; run current process");
    return run_probe();
}
