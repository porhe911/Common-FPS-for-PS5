/*
 * Common FPS v0.28b SR9A - PUI context-only bootstrap controller
...
extern "C" {
extern const unsigned char commonfps_sr9a_receiver_elf[];
extern const std::size_t commonfps_sr9a_receiver_elf_size;
...
constexpr const char* kLogPath = "/data/CommonFPS_SR9A_pui_context.log";
...
constexpr std::uint32_t kHealthMagic = 0x41394852U;
...
void log_injection(const common_fps::legacy_v028b::InjectionResult& r) noexcept {
    ...
    std::fprintf(f,
        "SR9A INJECT attached=%d elf_loaded=%d payload_args=%d bootstrap=%d pthread_ok=%d pthread_rc=%d trace_continue=%d trace_stop=%d imports_resolved=%u imports_unresolved=%u first_unresolved=%s\n",
...
void log_health(const HealthPacket& h) noexcept {
    ...
    if (h.kind == kHealthContext) {
        std::fprintf(f, "SR9A CONTEXT stage=%u mask=0x%04llx\n",
            h.loading, static_cast<unsigned long long>(h.sequence));
    } else if (h.kind == kHealthReady) {
        std::fprintf(f, "SR9A CONTEXT READY stage=%u mask=0x%04llx\n",
            h.loading, static_cast<unsigned long long>(h.sequence));
    } else if (h.kind == kHealthPacket) {
        std::fprintf(f, "SR9A HEALTH seq=%llu state=%s fps=%.1f\n",
...
public:
    ...
    bool wait_context_ready(unsigned milliseconds) noexcept {
        for (unsigned elapsed = 0; elapsed < milliseconds; elapsed += 20) {
            ...
            if (got == static_cast<ssize_t>(sizeof(h)) && h.magic == kHealthMagic) {
                log_health(h);
                if (h.kind == kHealthReady && h.sequence == kContextFullMask)
                    return true;
            }
            usleep(20000);
        }
...
private:
    void send(const PhufStatePacket& p) noexcept {
        ...
        if (n != static_cast<ssize_t>(sizeof(p))) {
            log_line("SR9A PHUF send failed");
            return;
        }
        ...
        log_line("SR9A HEALTH packet timeout");
    }
...
int run_probe() noexcept {
    ...
    log_pid("SR9A CHILD pid=", getpid());
    log_line("SR9A START PUI context-only ShellUI probe");
    log_line("SR9A NO CreateLabel / NO AppendChild / NO Detour / NO UI hook");
    log_line("SR9A backend+PHUF+bootstrap identical to proven SR8B");

    Wire wire;
    if (!wire.initialize()) {
        log_line("SR9A ABORT controller UDP init failed");
        return 1;
    }

    const pid_t shellui = wait_stable_shellui();
    log_pid("SR9A ShellUI stable pid=", shellui);
    if (shellui <= 0) return 2;

    const auto injection = inject_renderer_once(
        shellui, commonfps_sr9a_receiver_elf, commonfps_sr9a_receiver_elf_size);
    log_injection(injection);
    if (commonfps_v028b_import_unresolved_count() != 0) {
        log_line("SR9A ABORT fail-closed import resolver rejected receiver");
        return 3;
    }
    if (!injection.pthread_create_ok) {
        log_line("SR9A ABORT injection did not reach successful pthread_create");
        return 4;
    }
    if (!wire.wait_context_ready(12000)) {
        log_line("SR9A ABORT PUI context did not reach full READY mask");
        return 5;
    }

    log_line("SR9A PUI CONTEXT PASS; begin unchanged production PHUF lifecycle");
    ...
    for (int second = 0; second < kRuntimeSeconds; ++second) {
        ...
        if (pid != active_pid) {
            ...
            log_pid("SR9A CHANGE eboot.bin pid=", active_pid);
        }
        ...
        if (counter == 0) {
            ...
            if (!resolved) {
                log_line("SR9A DISCOVERY not ready; retry in 5s");
                retry = kDiscoveryRetrySeconds;
                wire.loading(); sleep(1); continue;
            }
            counter = *resolved; baseline = false; warmup = 1;
            log_line("SR9A COUNTER READY");
        }
    ...
    log_line("SR9A DONE controller clean return after 240s; context receiver remains until reboot");
    return 0;
}
}

extern "C" int main() {
    log_pid("SR9A PARENT start pid=", getpid());
    const pid_t child = fork();
    if (child > 0) {
        log_pid("SR9A PARENT forked child=", child);
        log_line("SR9A PARENT RETURN 0");
        return 0;
    }
    if (child < 0) log_line("SR9A fork failed; run current process");
    return run_probe();
}
