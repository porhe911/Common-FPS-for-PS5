/*
 * Common FPS v0.28b SR9H - Widget signature metadata probe
 * SPDX-License-Identifier: GPL-3.0-or-later
 *
 * Metadata only. No PUI method is invoked on a Widget and no UI state changes.
 */
...
#define SIGNATURE_PORT_NETWORK 0xFCD8u /* htons(55548) */
#define SIGNATURE_MAGIC 0x48394857u      /* "WH9H" */
...
static bool is_extra_target(const char* name) {
    static const char* targets[] = {
        "FindWidgetByName", "set_Visible", "set_Opacity", "set_X", "set_Y",
        "set_Name", "set_Width", "set_Height", "set_Position", "set_Size"
    };
...
int main(void) {
    ...
    send_packet(KIND_STAGE, 0, 0, "SR9H", "START_WIDGET_SIGNATURE_ONLY", "");
    ...
    MonoClass* widget = g_mono_class_from_name(image, "Sce.PlayStation.PUI.UI2", "Widget");
    if (!widget) {
        send_packet(KIND_ERROR, 0, 25, "Widget", "CLASS_NOT_FOUND", "");
        return 25;
    }
    send_packet(KIND_STAGE, 0, 0, "Widget", "CLASS_FOUND", "");

    unsigned found = 0;
    void* iter = 0;
    for (;;) {
        MonoMethod* method = g_mono_class_get_methods(widget, &iter);
        if (!method) break;
        ...
        /* Main goal: every zero-argument instance method. Also keep a few
         * known one-argument properties/finders for later context. */
        if (count != 0 && !is_extra_target(name)) continue;
    ...
    send_packet(KIND_DONE, 0, found, "SR9H", "DONE", "");
    close(g_socket);
    return 0;
}
