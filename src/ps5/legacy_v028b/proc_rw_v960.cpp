...
// Values recovered from the exact stable v0.28b firmware table and checked
// against PS5 Payload SDK v0.41 for FW 9.60.
...
std::uintptr_t find_proc(pid_t pid) noexcept {
    if (pid <= 0 || !fw_is_960() || KERNEL_ADDRESS_DATA_BASE == 0)
        return 0;
    ...
    for (unsigned i = 0; current != 0 && i < kMaxProcWalk; ++i) {
        ...
        if (!read_kernel_value(current + kProcNext, next))
            return 0;
        if (next == current)
            return 0;
        current = next;
...
std::uintptr_t choose_dmap(
    ...
    // The reference payload prefers a plausible VA-PA derived DMAP value and
    // otherwise falls back to the fixed per-firmware table value.
