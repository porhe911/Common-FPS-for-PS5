/*
 * Common FPS v0.28b SR8B - UDP-health receiver bootstrap probe
 * SPDX-License-Identifier: GPL-3.0-or-later
 *
 * The FPS backend and PHUF wire are the hardware-proven SR7 path.  The only
 * new variable is the one-time ShellUI bootstrap of a minimal, UI-free PHUF
 * receiver.  Receiver health is reported only through UDP 55542; no ShellUI
...
extern "C" {
extern const unsigned char commonfps_sr8b_receiver_elf[];
extern const std::size_t commonfps_sr8b_receiver_elf_size;
...
constexpr const char* kLogPath = "/data/CommonFPS_SR8B_receiver_udp.log";
...
constexpr std::uint32_t kHealthMagic = 0x42384852U;
...
void log_injection(const common_fps::legacy_v028b::InjectionResult& r) noexcept {
    ...
    std::fprintf(f,
        "SR8B INJECT attached=%d elf_loaded=%d payload_args=%d bootstrap=%d pthread_ok=%d pthread_rc=%d trace_continue=%d trace_stop=%d imports_resolved=%u imports_unresolved=%u first_unresolved=%s\n",
...
void log_health(const HealthPacket& h) noexcept {
    ...
    if (h.kind == kHealthReady) {
        std::fprintf(f, "SR8B HEALTH receiver READY via UDP\n");
    } else {
        std::fprintf(f, "SR8B HEALTH seq=%llu state=%s fps=%.1f\n",
    ...
    bool wait_ready(unsigned milliseconds) noexcept {
        for (unsigned elapsed = 0; elapsed < milliseconds; elapsed += 20) {
            ...
            if (got == static_cast<ssize_t>(sizeof(h)) &&
                h.magic == kHealthMagic && h.kind == kHealthReady) {
                log_health(h);
                return true;
            }
            usleep(20000);
...
private:
    void send(const PhufStatePacket& p) noexcept {
        ...
        if (n != static_cast<ssize_t>(sizeof(p))) {
            log_line("SR8B PHUF send failed");
            return;
        }
        ...
        log_line("SR8B HEALTH packet timeout");
    }
...
int run_probe() noexcept {
    ...
    log_pid("SR8B CHILD pid=", getpid());
    log_line("SR8B START UDP-health receiver-only ShellUI bootstrap");
    log_line("SR8B NO files / NO PUI / NO Mono / NO UI hook / one injection only");
    log_line("SR8B backend+PHUF identical to proven SR7");

    Wire wire;
    if (!wire.initialize()) {
        log_line("SR8B ABORT controller UDP init failed");
        return 1;
    }

    const pid_t shellui = wait_stable_shellui();
    log_pid("SR8B ShellUI stable pid=", shellui);
    if (shellui <= 0) return 2;

    const auto injection = inject_renderer_once(
        shellui, commonfps_sr8b_receiver_elf, commonfps_sr8b_receiver_elf_size);
    log_injection(injection);

    if (commonfps_v028b_import_unresolved_count() != 0) {
        log_line("SR8B ABORT fail-closed import resolver rejected receiver");
        return 3;
    }
    if (!injection.pthread_create_ok) {
        log_line("SR8B ABORT injection did not reach successful pthread_create");
        return 4;
    }
    if (!wire.wait_ready(10000)) {
        log_line("SR8B ABORT no UDP READY from injected receiver");
        return 5;
    }

    log_line("SR8B RECEIVER READY PASS; begin production PHUF lifecycle");
    ...
    for (int second = 0; second < kRuntimeSeconds; ++second) {
        ...
        if (pid != active_pid) {
            ...
            seconds_on_pid = 0;
            retry = 0;
            log_pid("SR8B CHANGE eboot.bin pid=", active_pid);
        }
        ...
        if (counter == 0) {
            ...
            if (!resolved) {
                log_line("SR8B DISCOVERY not ready; retry in 5s");
    ...
    log_line("SR8B DONE controller clean return after 240s; receiver remains until reboot");
    return 0;
}
}

extern "C" int main() {
    log_pid("SR8B PARENT start pid=", getpid());
    const pid_t child = fork();
    if (child > 0) {
        log_pid("SR8B PARENT forked child=", child);
        log_line("SR8B PARENT RETURN 0");
        return 0;
    }
    if (child < 0)
        log_line("SR8B fork failed; run current process");
    return run_probe();
}
