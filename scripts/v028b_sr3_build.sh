#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
export PS5_PAYLOAD_SDK="${PS5_PAYLOAD_SDK:-/opt/ps5-payload-sdk}"
ETAHEN_SRC="${ROOT}/.deps/etahen"
SHSRV_SRC="${ROOT}/.deps/shsrv"
OUT="${ROOT}/dist/v028b-sr3"

rm -rf "${ROOT}/build-v028b-sr3"
mkdir -p "${OUT}"
rm -f "${OUT}/Common_FPS_SR3_v028b_V5_ModuleList_DMAP.elf" \
      "${OUT}/Common_FPS_SR3_v028b_V5_ModuleList_DMAP_etaHEN.plugin" \
      "${OUT}/SHA256SUMS.txt" "${OUT}/LINKED_SYMBOLS.txt"

"${PS5_PAYLOAD_SDK}/bin/prospero-cmake" \
  -S "${ROOT}/ps5" -B "${ROOT}/build-v028b-sr3" \
  -DCMAKE_BUILD_TYPE=Release \
  -DCOMMON_FPS_ETAHEN_SOURCE="${ETAHEN_SRC}" \
  -DCOMMON_FPS_SHSRV_SOURCE="${SHSRV_SRC}"

cmake --build "${ROOT}/build-v028b-sr3" --target common_fps_v028b_sr3 -j"$(nproc)"

ELF="${OUT}/Common_FPS_SR3_v028b_V5_ModuleList_DMAP.elf"
PLUGIN="${OUT}/Common_FPS_SR3_v028b_V5_ModuleList_DMAP_etaHEN.plugin"
test -f "${ELF}"
test -f "${PLUGIN}"

# SR3 must remain free of failed RC transports and visual injection code.
if strings "${ELF}" | grep -Eq 'pt_attach|pt_copyout|pt_detach|mdbg_copyout|SceShellUI|elfldr'; then
  echo "ERROR: SR3 contains forbidden ptrace/MDBG/renderer symbol"
  exit 1
fi

nm -C "${ELF}" > "${OUT}/LINKED_SYMBOLS.txt" || true
for sym in find_game_pid_sysctl translate proc_read; do
  if ! grep -q "${sym}" "${OUT}/LINKED_SYMBOLS.txt"; then
    echo "ERROR: SR3 missing recovered backend symbol: ${sym}"
    exit 1
  fi
done

for marker in \
  'SR3 MODULE list-size' \
  'SR3 AUTH debugger active for v5 module-list discovery only' \
  'SR3 AUTH restored before DMAP reads' \
  'SR3 DMAP table read OK' \
  'SR3 COUNTER READY'; do
  if ! strings "${ELF}" | grep -q "${marker}"; then
    echo "ERROR: SR3 diagnostic marker missing: ${marker}"
    exit 1
  fi
done

sha256sum "${ELF}" "${PLUGIN}" > "${OUT}/SHA256SUMS.txt"
cat "${OUT}/SHA256SUMS.txt"
