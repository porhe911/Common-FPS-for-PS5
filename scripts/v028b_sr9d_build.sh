...
OUT="${ROOT}/dist/v028b-sr9d"

rm -rf "${ROOT}/build-v028b-sr9d"
mkdir -p "${OUT}"
rm -f "${OUT}"/*

"${PS5_PAYLOAD_SDK}/bin/prospero-cmake" \
  -S "${ROOT}/ps5/sr9d" -B "${ROOT}/build-v028b-sr9d" \
  -DCMAKE_BUILD_TYPE=Release \
  -DCOMMON_FPS_ETAHEN_SOURCE="${ETAHEN_SRC}"

cmake --build "${ROOT}/build-v028b-sr9d" --target common_fps_v028b_sr9d -j"$(nproc)"

RECEIVER="${OUT}/Common_FPS_SR9D_Dispatcher_Signature_Receiver.elf"
ELF="${OUT}/Common_FPS_SR9D_v028b_Dispatcher_Signature.elf"
PLUGIN="${OUT}/Common_FPS_SR9D_v028b_Dispatcher_Signature_etaHEN.plugin"
...
if strings "${RECEIVER}" | grep -Eqi 'CreateLabel|AppendChild|Set_Property|TextColor|FitWidthToText|DetourFunction|WriteJump|PatchInJump|OnRender_Hook|pt_attach|pt_copyout|mdbg_copyout'; then
  echo "ERROR: SR9D receiver contains forbidden UI mutation/hook/transport code"
  exit 1
fi

for marker in \
  'START_SIGNATURE_ONLY' \
  ...
  strings "${RECEIVER}" | grep -Fq "${marker}" || {
    echo "ERROR: SR9D signature marker missing: ${marker}"
    exit 1
  }
done
...
if [[ -n "${BAD_RELOCS}" ]]; then
  echo "ERROR: SR9D receiver has unsupported relocations:"
  echo "${BAD_RELOCS}"
  exit 1
fi
...
for symbol in \
  ...
  grep -q "${symbol}" "${OUT}/LINKED_SYMBOLS.txt" || {
    echo "ERROR: SR9D missing required controller symbol: ${symbol}"
    exit 1
  }
done

for marker in \
  'SR9D START targeted PUI dispatcher signature probe' \
  'NO CreateLabel / NO AppendChild / NO PUI mutation / NO Detour / NO code patch' \
  'SR9D SIGNATURE name=' \
  'SR9D PASS signature capture complete' \
  'imports_unresolved='; do
  strings "${ELF}" | grep -Fq "${marker}" || {
    echo "ERROR: SR9D controller marker missing: ${marker}"
    exit 1
  }
done
